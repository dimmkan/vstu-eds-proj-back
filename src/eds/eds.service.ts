...
export class EdsService {
  ...
  async getFileById(id: number): Promise<EdsEntity> {
    return await this.edsRepository.findOne(
      { id },
      { select: ['fileData', 'fileName', 'fileType', 'fileSize'] },
    );
  }
  ...
  async cronEds30() {
    const result = await getRepository(EdsEntity)
      ...
      .where(
        'TIMESTAMPDIFF(DAY, NOW(), `toDate`) < 30 AND TIMESTAMPDIFF(DAY, NOW(), `toDate`) > 13',
      )
      .getMany();
    ...
    for (const element of result) {
      ...
      const interval = Math.ceil(
        (time.getTime() - dateNow.getTime()) / (1000 * 60 * 60 * 24),
      );
  ...
  async addOpenPartFile(file: Express.Multer.File, id: number) {
    const eds = await this.edsRepository.findOne(id);
  ...
  async deleteOpenPartFile(id: number) {
    const eds = await this.edsRepository.findOne(id);
  ...
  async getOpenPartFileById(id: number) {
    return await this.edsRepository.findOne(
      { id },
      {
        select: [
