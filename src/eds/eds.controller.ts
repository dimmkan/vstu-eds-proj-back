...
export class EdsController {
  ...
  @Put('update/:id')
  async updateEds(
    @Body() updateEdsDto: UpdateEdsDto,
    @Param('id') id: number,
  ): Promise<EdsEntity> {
  ...
  @Post('addopenpartfile/:id')
  @UseInterceptors(FileInterceptor('file'))
  async uploadOpenPartFile(
    @UploadedFile() file: Express.Multer.File,
    @Param('id') id: number,
  ): Promise<void> {
  ...
  @Delete('deleteopenpartfile/:id')
  async deleteOpenPartFile(@Param('id') id: number): Promise<void> {
    await this.edsService.deleteOpenPartFile(id);
  }

  @Get('downloadopenpart/:id')
  async downloadOpenPartFile(
    @Res({ passthrough: true }) res: Response,
    @Param('id') id: number,
  ): Promise<StreamableFile> {
